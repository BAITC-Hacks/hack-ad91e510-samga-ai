...
let choices = scenarios.find(s => s.id === 'coverage').choices;
let result = evaluate(choices);
...
function render(){
  ...
  $('outcome-note').textContent='Критических показателей: '+data.critical+' · порог ниже 40';
  $('score-delta').textContent = mode === 'before' ? 'Исходное состояние' : `${signed(result.score - baseline.score)} к индексу`;
...
const simulator=mountSimulator({
 ...
 showImpact:()=>{setMode("after");setLayer("districts");}
});
function renderDecisions(){simulator.renderPlan();}
render();
...
async function init(){
  try{
    ...
    map.on('load',()=>{
      ...
      $('map-status').className='map-status loaded';mapReady=true;window.cityDemo={map,get baseline(){return baseline},get result(){return result},getState:()=>({mode,selected,activeLayer,choices:structuredClone(choices)})};updateDistricts();
