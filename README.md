...
**Текущий этап: серверный расчёт, поиск стратегий, сравнение и проверяемый помощник.**
В репозитории есть аналитический сайт с четырьмя фиксированными сценариями и три интерактивных прототипа.
Рабочий экран запускается командой node backend/server.mjs на http://127.0.0.1:3001. Живой AI-доступ команды подключается позже;
этот этап не является завершённым конкурсным продуктом.

## Проверяемая аналитика и помощник

Запуск: `node backend/server.mjs` (Node.js 24+), затем http://127.0.0.1:3001. Расчёт, сравнение, поиск и источники работают без ключа. [Контракт, точность, проверки и ограничения](analytics/README.md).

## Интерактивные прототипы

Откройте [index.html](index.html) непосредственно в браузере: **карта города**, **лаборатория стратегий** и **рабочая панель**. Они поддерживают ручной выбор пяти мер, районы, бюджет, ограничения и расчёт последствий. Интернет и установка пакетов для этих трёх автономных HTML не требуются.

[Руководство по прототипам](docs/PROTOTYPES.md) описывает демонстрацию, контрольные числа и повторяемый поиск стратегий. Карта схематическая, AI-обоснование демонстрационное. Полный перебор учебной модели реализован отдельно в Python + NumPy. Новая галерея из десяти концепций ещё не завершена.
