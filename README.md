...
Веб-панель, сервер, поиск, отчёты и мобильные исходники собраны в этом репозитории. Не подключены синхронизация мобильных ролей, реальные городские службы и авторизация; голос пока ошибается. Для live AI на другой машине нужны настроенный API-доступ и ключи в памяти процесса. Проверка чистой локальной копии не заменяет проверку чужого ноутбука и подтверждение сдачи на платформе.
...
В финальной общей сборке прошли **123/123 теста**, **16/16 HTTP-проверок сценария эксперта** и **5/5 тестов мобильной модели жителя**. В браузере подтверждены 83 / 55,61, перенос в отчёт, скачивание и возврат с теми же мерами, а также мобильная галерея. [Машиночитаемый результат](docs/user-guide/verification.json). Текстовый AI подтверждён в задаче ассистента; голос, другой ноутбук, доступ эксперта и финальная сдача — отдельные незавершённые шаги.
