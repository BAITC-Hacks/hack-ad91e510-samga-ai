...
Backend принимает пять решений и готовый simulator JSON; OpenAI-интеграция реализована, но живой вызов с ключом команды ещё не проверен. Пользовательский выбор мер и подключение frontend к API ещё не завершены;
...
2. Подключить frontend к backend/API и проверить живой AI-анализ с ключом и моделью команды.
...
- [ ] Завершить пользовательский выбор пяти решений, подключение frontend к API и проверку живого AI.
...
## Backend/API — Меру, ветка meru/ai

Реализованы POST /api/simulate (обёртка над engine Карима), POST /api/analyze
(готовые расчёты → OpenAI), health, контракт v1, обработка ошибок и явный demo-режим.
OpenAI не рассчитывает Score: числовой результат берётся только из simulator JSON.

[Запуск, интеграция frontend, контракты и ограничения](backend/README.md).
Node.js 24+, внешние пакеты не нужны. Быстрый запуск из корня без ключа:

```sh
node --env-file=backend/.env.example backend/server.mjs
```
...
AI_MODE=demo явно помечает шаблонный ответ. Для живого анализа нужны серверные
OPENAI_API_KEY и OPENAI_MODEL, инструкции в backend/README.md.
