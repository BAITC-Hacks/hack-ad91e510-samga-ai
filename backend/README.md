...
его за AI и не обращается в сеть. Сервер слушает только 127.0.0.1:4197.
...
берётся из повторно рассчитанного сервером сценария. В схеме AI-ответа числовых полей нет.
...
async function post(path, data) {
  const response = await fetch('http://127.0.0.1:4197' + path, {
    method: 'POST',
...
http://127.0.0.1:4197. localhost и 127.0.0.1 — разные origin.
...
Это локальный backend для командной демонстрации. Основная карта доступна на корневом маршруте; дополнительный экран аналитики — на /analytics/. Главная карта интегрирована через контракт v1.
