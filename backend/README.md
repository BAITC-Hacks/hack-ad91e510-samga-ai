...
- `ai.mjs` передаёт готовый JSON в OpenAI и проверяет формат объяснения.
...
Промпт запрещает вычисления, придуманные эффекты и числовые прогнозы.
Рекомендации являются гипотезами, пока simulator не оценит новый набор.
Проверка структуры ответа не доказывает фактическую верность текста AI.
...
Это локальный backend для командной демонстрации. Frontend пока не подключён.
Числа принятого /api/analyze JSON считаются доверенным результатом simulator:
проверяются структура и допустимость choices, но не подлинность или соответствие чисел choices.
